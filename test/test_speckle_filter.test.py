...
def generate_test_description():
    ...
    dist_node = launch_ros.actions.Node(
        ...
        parameters = [config],
        remappings=[
            ('/scan_filtered', '/scan_filtered_distance'),
        ]
        )
    ...
    eucl_node = launch_ros.actions.Node(
        ...
        parameters = [config],
        remappings=[
            ('/scan_filtered', '/scan_filtered_euclidean'),
        ]
        )
...
class TestFixture(Node):
    ...
    def start_subscribers(self):
        self.distance_subscriber = self.create_subscription(
            LaserScan,
            'scan_filtered_distance',
            self.dist_cb,
            10
        )

        self.euclidean_subscriber = self.create_subscription(
            LaserScan,
            'scan_filtered_euclidean',
            self.euclid_cb,
            10
        )
